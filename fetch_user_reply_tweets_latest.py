...
async def username_handler(event):
    ...
    latest_activity = await fetch_latest_activity(username)
    if latest_activity:
        await event.respond(f"Latest tweets and replies from @{username}:\n\n" + latest_activity)
    else:
        await event.respond(f"Could not fetch activity for @{username}. Please try again.")
...
async def fetch_latest_activity(username):
    headers = {
        "Authorization": f"Bearer {bearer_token}"
    }
    ...
    if user_response.status_code == 200:
        ...
        if user_id:
            tasks = [
                fetch_tweets(user_id, headers),
                fetch_user_replies(user_id, headers)
            ]
            tweets, user_replies = await asyncio.gather(*tasks)
            messages = []

            for tweet in tweets:
                messages.append(f"Tweet: {tweet['text']}")
                replies = await fetch_replies_to_tweet(tweet['id'], headers)
