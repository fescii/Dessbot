...
async def send_telegram_message(message):
    for chat_id in get_user_chat_ids():
        await telegram_client.send_message(chat_id, message)

async def fetch_and_send_tweet(tweet_id):
...
if __name__ == "__main__":
    ...
    tweet_ids = [
        "1850922809213735091",  # Replace with actual tweet IDs you want to lookup
        "1837926063894155299"
    ]

    loop = asyncio.get_event_loop()
    for tweet_id in tweet_ids:
        loop.run_until_complete(fetch_and_send_tweet(tweet_id))
